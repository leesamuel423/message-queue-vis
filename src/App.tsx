...
import { Message, MessageType, AckStatus, Stats } from './types'
...
function App() {
  ...
  // State
  const [processingSpeed, setProcessingSpeed] = useState<number>(1);
  ...
  const [producerAnimating, setProducerAnimating] = useState<boolean>(false);
  
  // Unique ID for messages
  const [messageCounter, setMessageCounter] = useState<number>(1);

  // Message Queue Service
  // Not used directly but kept as example for implementation
  ...
  // Update statistics
  const updateStats = (type: 'add' | 'process', success?: boolean): void => {
    setStats(prevStats => {
  ...
  const triggerProducerAnimation = (): void => {
  ...
  const addMessage = (type: MessageType): void => {
  ...
  const handleSendSuccess = (): void => {
    addMessage('success');
  };

  const handleSendFail = (): void => {
    addMessage('fail');
  };
